   ...
   public void directKinematic(){
       ...
       if (d < 2 * r){
         ...
         // half distance between tool positions
           double h = Math.sqrt( (r * r) + (Math.pow((d / 2), 2)) / 2);
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double alpha = Math.PI - Math.atan2(dy1, dx1);
        // elbows positions
        xj1 = xa + h1 * Math.cos((Math.PI / 2) - alpha);
        yj1 = ya + h1 * Math.sin((Math.PI / 2) - alpha);
        ...
        double l2 = d2 / 2;
        double h2 = Math.sqrt(Math.pow(r, 2) - Math.pow(d2, 2) / 4);

        xa = xm2 + 0.5 * (dx2);
        ya = ym2 + 0.5 * (dy2);

        double alpha2 = Math.PI - Math.atan2(dy2, dx2);

        // elbows positions
        xj2 = xa - h2 * Math.cos((Math.PI / 2) - alpha2);
        yj2 = ya - h2 * Math.sin((Math.PI / 2) - alpha2);
    ...
    public int get_pwm1(){
        int pwm = 0;
        pwm = (int) ((10.4684231172*theta1)+242.0302693515);
        return pwm;
    }
    // ditto for motor 2
    public int get_pwm2(){
